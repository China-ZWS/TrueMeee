...
@implementation DDURLNavgation

DEFINE_SINGLETON_FOR_M(DDURLNavgation)
...
+ (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated replace:(BOOL)replace
{
    if (!viewController) {
        ...
        if([viewController isKindOfClass:[UINavigationController class]]) {
            [DDURLNavgation setRootViewController:viewController];
        } // 如果是导航控制器直接设置为根控制器
        else {
            UINavigationController *navigationController = [DDURLNavgation sharedDDURLNavgation].currentNavigationViewController;
            if (navigationController) { // 导航控制器存在
                ...
                [DDURLNavgation sharedDDURLNavgation].applicationDelegate.window.rootViewController = navigationController;
...
+ (void)presentViewController:(UIViewController *)viewController animated: (BOOL)flag completion:(void (^ __nullable)(void))completion
{
    if (!viewController) {
        ...
        UIViewController *currentViewController = [[DDURLNavgation sharedDDURLNavgation] currentViewController];
        if (currentViewController) { // 当前控制器存在
            ...
            [DDURLNavgation sharedDDURLNavgation].applicationDelegate.window.rootViewController = viewController;
        }
    }
}
...
+ (void)popTwiceViewControllerAnimated:(BOOL)animated {
    [DDURLNavgation popViewControllerWithTimes:2 animated:YES];
}
...
+ (void)popToRootViewControllerAnimated:(BOOL)animated {
    UIViewController *currentViewController = [[DDURLNavgation sharedDDURLNavgation] currentViewController];
    NSUInteger count = currentViewController.navigationController.viewControllers.count;
    [DDURLNavgation popViewControllerWithTimes:count-1 animated:YES];
}
...
+ (void)dismissViewControllerWithTimes:(NSUInteger)times animated: (BOOL)flag completion: (void (^ __nullable)(void))completion {
    UIViewController *rootVC = [[DDURLNavgation sharedDDURLNavgation] currentViewController];
